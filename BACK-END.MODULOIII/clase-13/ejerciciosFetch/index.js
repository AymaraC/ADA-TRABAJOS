/*Ejercicio 1: Consultar Información de una API Pública. Descripción: 
1. Crea un archivo index.js. 
2. Usando el método fetch en Node.js, realiza una solicitud a la API de Rick and Morty. 
3. Extrae el nombre del primer personaje de la respuesta y muéstralo en la consola. Pistas: 
...
Direccion de API: https://rickandmortyapi.com/api/character/ */

const url = 'https://rickandmortyapi.com/api/character/'

fetch(url)
.then(res => res.json())        // Convertimos el body a json
.then(data => {
    console.log("Primer personaje: ", data.results[1].name);
    console.log("Status: ", data.results[1].status);
})
