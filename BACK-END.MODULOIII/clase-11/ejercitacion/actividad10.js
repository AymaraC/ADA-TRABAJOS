...
Desarrolla una función que simule la actualización de un inventario después de una compra. Si el inventario es 
insuficiente (cantidad <= 0), la actualización debe fallar y lanzar un error. Especificaciones:
...
async function inventario() {
    console.log(`...`)
    try {
        const i = await actualizarInventario(5, 'Manzanas')
        console.log(i);
...
function actualizarInventario(cantidad, producto) {
    return new Promise((resolve,reject) => {
       setTimeout(() => {
         if(cantidad > 0) {
            resolve(`✔️  Inventario actualizado. Ingreso: ${cantidad} ${producto}`)
            return;
        }
