...
Esta función debe tardar 3 segundos en completarse y retornar un mensaje confirmando que el correo de recuperación ha sido
enviado.
Especificaciones:
...
Este ejercicio muestra cómo se manejan operaciones asíncronas en procesos de recuperación de contraseñas, una 
funcionalidad común en aplicaciones web.*/

function recuperarContrasena(correo) {
    return new Promise((resolve, reject) => {
        if(!correo) {
            reject(`❌ No se ingresó ningún correo.`)
            return;
        } 
        
            setTimeout(() => {
                resolve(`Correo de recuperación enviado correctamente a: ${correo}`)
            }, 3000)
    })
}
...
async function iniciarRecuperacion() {
    console.log(`⌛`)
    try{
    const contraseña = await recuperarContrasena('manuel@example.com');
